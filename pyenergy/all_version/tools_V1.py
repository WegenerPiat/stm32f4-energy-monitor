...
def continuous(serial, mpoints, delay=0.1):
    ...
    for mp in mpoints:
        em.enableMeasurementPoint(mp)
        em.start(mp)
        print "energy_{0}, avg_power_{0}, ins_voltage_{0}, avg_voltage_{0},".format(mp),	# 
    print ""
    ...
    try:
        while True:
            ...
            for mp in mpoints:
                ...
                if first:
                    print m.time,
                print ", {}, {}, {}, {},".format(m.energy, m.avg_power, m.test_tension, m.avg_voltage),	#m.energy/m.time m.avg_current,
            print ""
            sleep(delay)
