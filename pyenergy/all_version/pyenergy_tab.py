...
class MeasurementSet(object):
    def __init__(self, measurements):
        ...
        self.donnee1       	= sum(self.measurements).donnee1
        self.elapsed_time1  = sum(self.measurements).elapsed_time1
        self.donnee2   		= sum(self.measurements).donnee2  
        self.elapsed_time2 	= sum(self.measurements).elapsed_time2
        self.donnee3 		= sum(self.measurements).donnee3
        self.elapsed_time3  = sum(self.measurements).elapsed_time3
        self.donnee4  		= sum(self.measurements).donnee4
        self.elapsed_time4  = sum(self.measurements).elapsed_time4


    def _op_self(self, other, fn):
        params = ["donnee1", "elapsed_time1", "donnee2", "elapsed_time2", "donnee3",
                "elapsed_time3", "donnee4", "elapsed_time4"]
    ...
    def __repr__(self):
        keys = ["donnee1", "elapsed_time1", "donnee2", "elapsed_time2", "donnee3", "elapsed_time3", "donnee4", "elapsed_time4"]
...
class Measurement(object):
    ...
    def __init__(self, donnee1=0, elapsed_time1=0, donnee2=0, elapsed_time2=0, donnee3=0,
                elapsed_time3=0, donnee4=0, elapsed_time4=0):
        self.donnee1		= float(donnee1)
        self.elapsed_time1	= float(elapsed_time1)
        self.donnee2		= float(donnee2)
        self.elapsed_time2 	= float(elapsed_time2)
        self.donnee3		= float(donnee3)
        self.elapsed_time3	= float(elapsed_time3)
        self.donnee4		= float(donnee4)
        self.elapsed_time4  = float(elapsed_time4)
...
class EnergyMonitor(object):
    ...
    #MeasurementData = namedtuple('MeasurementData', ' energy_accum elapsed_time peak_power peak_voltage peak_current n_samples test_tension avg_voltage')	#avg_current
    #MeasurementData_packing = "=QQLLLLQQ"
    MeasurementData = namedtuple('MeasurementData', 'donnee1 elapsed_time1 donnee2 elapsed_time2 donnee3 elapsed_time3 donnee4 elapsed_time4')
    MeasurementData_packing = "=QQQQQQQQ"

    InstantaneousData = namedtuple('InstantaneousData', 'voltage current_time mpointed')	# current average_voltage average_current
    InstantaneousData_packing = "=LQL"
    ...
    port_mappings = {1: ["PC4"], 2: ["PC5"], 3: ["PC2"], 4:["PC1"]}	#, "PA1"
    ...
    def __init__(self, serial="EE00"):
        ...
        # Set up default parameters for measurement point 1 and 4
        defaultparams = {'resistor':0.04, 'gain':20, 'vref':12}
        self.measurement_params = {i : copy(defaultparams) for i in [1,4]}
        # Set up an other default parameters for measurement point 2 and 3
        self.measurement_params[2] = {'resistor':0.2, 'gain':20, 'vref':12}
	self.measurement_params[3] = {'resistor':0.2, 'gain':20, 'vref':12}
    ...
    def enableMeasurementPoint(self, m_point, adc=None):
        ...
        if adc is None:
            # If we want mpoint 3 or 4 and ADC3 is free, prioritise it
            #  because ADC3 doesnt work with mpoint 1 or 2
            if m_point in [3,4] and self.adcMpoint[2] is None:
                adc = 2
            else:
                adc = self.adcMpoint.index(None)
        if m_point in [1,2] and adc == 2:
    ...
    def convertData(self, md, resistor, gain, vref, samplePeriod):
        ...
        try:
	    	...
	    	#tension = md.test_tension/4096.*vref
            #ap = en/el

	    tension1 = md.donnee2/4096.*vref
            el1 = md.elapsed_time1 * 2. / 168000000 * 2
            tension2 = md.donnee2/4096.*vref
            el2 = md.elapsed_time2 * 2. / 168000000 * 2
	    tension3 = md.donnee3/4096.*vref
            el3 = md.elapsed_time3 * 2. / 168000000 * 2
	    tension4 = md.donnee4/4096.*vref
            el4 = md.elapsed_time4 * 2. / 168000000 * 2

        except ZeroDivisionError:
            el1, el2, el3, el4, tension1, tension2, tension3, tension4  = 0, 0, 0, 0, 0, 0, 0, 0

        m = Measurement(el1, tension1, el2, tension2, el3, tension3, el4, tension4)
	...
	#print"taile du paquet: ", calcsize(EnergyMonitor.MeasurementData_packing)
    ...
    def getInstantaneous(self, m_point=1):
        ...
        """
	print "taille paquet: ", calcsize(EnergyMonitor.InstantaneousData_packing)
    ...
    def debugInstantaneous(self, v):
        ...
        """
        mp = v[3]
	print "point de mesure : ", v[3]
	print "voltage brut : ", v[0]
        resistor = self.measurement_params[mp]['resistor']
        gain = self.measurement_params[mp]['gain']
        vref = self.measurement_params[mp]['vref']

        print "Timestamp:", v[1] * 2. / 168000000 * 2
	print " temps ecoulé: ", float(v[1] / 168000000)/1000000000
        print "Current:  Raw={:4d}  Voltage@{}={:1.3f}V  Res Vdrop={:1.5f}V  Current={:1.5f}A".format(mp,
            EnergyMonitor.port_mappings[mp][0],
            v[0]/4096.*vref,
            float(vref) / gain / 4096. * v[0],
            float(vref) / gain / resistor / 4096. * v[0])
        print ""

    # create a text file to storing data
    def testWriteFile(self,m_point):
	"""
	   Permet d'écrire les données dans un fichier texte afin de les
	   utilisées.
	   Can write the data to a text file to the used
	"""
#d'abord tester si le fichier existe a la premiere lecture dans une fonction
#ecrire la donnee dans un fichier
#toujours ecrire a la suite

        firstwrite = True

	if firstwrite == True :
	    nbFichier = 1
	    nomDeFichier = "Donnee_conso_{}.txt".format(nbFichier)
	    testName = os.path.exists(nomDeFichier)
	    while testName == True :
		nbFichier += 1
	        nomDeFichier = "Donnee_conso_{}.txt".format(nbFichier)
		testName = os.path.exists(nomDeFichier)
	    ...
	    with open(nomDeFichier,"w") as fichier :
		fichier.write("fichier de Donnee de consomation numéro {}".format(nbFichier))
		fichier.write("\n")
	    firstwrite == False
