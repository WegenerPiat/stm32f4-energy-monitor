...
class MeasurementSet(object):
	...
	self.test_tension = sum(self.measurements).test_tension
    ...
    def _op_self(self, other, fn):
        params = ["energy", "time", "peak_power", "peak_voltage", "peak_current",	
                "n_samples", "avg_voltage", "test_tension", "avg_power"]
    ...
    def __repr__(self):
        keys = ["energy", "time", "peak_power", "peak_voltage", "peak_current", "n_samples", "avg_voltage", "test_tension", "avg_power"]	
...
class Measurement(object):
    ...
    def __init__(self, energy=0, time=0, peak_power=0, peak_voltage=0, peak_current=0,	
                n_samples=0, avg_voltage=0, test_tension=0, avg_power=0):
	...
	self.test_tension = float(test_tension)
...
class EnergyMonitor(object):
    ...
    MeasurementData = namedtuple('MeasurementData', ' energy_accum elapsed_time peak_power peak_voltage peak_current n_samples test_tension avg_voltage')
    MeasurementData_packing = "=QQLLLLQQ"


    InstantaneousData = namedtuple('InstantaneousData', 'voltage current_time mpointed')
    ...
    port_mappings = {1: ["PC4"], 2: ["PC5"], 3: ["PC2"], 4:["PC1"]}
    ...
    def convertData(self, md, resistor, gain, vref, samplePeriod):
	    ...
	    tension = md.test_tension/4096.*vref
            ...
            en, el, pp, pv, pi, av, tension, ap = 0, 0, 0, 0, 0, 0, 0, 0

        m = Measurement(en, el, pp, pv, pi, md.n_samples, av, tension, ap)
